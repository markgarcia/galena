...
window_render_surface::window_render_surface(const window& window, renderer& renderer) {
    if(renderer.type == renderer::renderer_type::dx11) {
        m_impl = std::make_unique<dx11_window_render_surface>(window, static_cast<dx11_renderer&>(renderer.get_impl()));
    }
}
...
unsigned int window_render_surface::get_width() const {
    return m_impl->get_width();
}

unsigned int window_render_surface::get_height() const {
